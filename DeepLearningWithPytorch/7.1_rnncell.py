...
# BucketIterator()는 데이터로더와 쓰임새가 같음. 즉, 배치 크기 단위로 값을 차례대로 꺼내어
# 메모리로 가져오고 싶을 때 사용
...
def training(epoch, model, trainloader, validloader):
    ...
    for b in trainloader:
        ...
        x, y = b.text, b.label
        # 꺼내 온 데이터가 cpu을 사용할 수 있도록 장치 지정
        # 반드시 모델과 같은 강치를 사용하도록 지정해야 함
        x, y = x.to(device), y.to(device)
        y_pred = model(x)
        # cross entropyLoss 손실 함수를 이용하여 오차 계산
    ...
    # 누적된 오차를 전체 데이터넷으로 나누어서 에포크 단계마다 오차를 구함
...
for epoch in range(epochs):
    ...
    # 훈련 데이터셋을 모덜에 적용했을 때의 정확도
    train_acc.append(epoch_acc)
    # 검증 데이터셋을 모덜에 적용했을 때의 오차
    valid_loss.append(epoch_valid_loss)
    # 검증 데이터셋을 모덜에 적용했을 때의 정확도
