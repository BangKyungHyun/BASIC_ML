...
# 학습을 마치면 가중치 파라미터가 담긴 파일이 torch.save 함수를 활용하여 피클 형태로 저장되어 있을 것입니다.
# 그럼 이제 해당 모델 파일을 불러와서 추론 및 평가를 수행하는 코드를 구현해야합니다.
...
# API 서버 등에서 랩핑wrapping하는 형태로 구현할 수 있을 것입니다.
...
# 디바이스로 로딩될 것입니다.
# 같은 컴퓨터라면 크게 상관이 없을 수도 있지만, 만약 다른 컴퓨터인데 GPU가 없거나 갯수가 다르다면 문제가 될 수 있습니다.
...
# load_state_dict는 json 형태의 모델 가중치가 저장된 객체를 실제 모델 객체에 로딩하는 함수입니다.
...
# 즉, 이렇게 생성된 model 객체는 임의로 초기화(random initialized)된 가중치 파라미터 값을 가지고 있을텐데요.
# 이것을 load_state_dict 함수를 통해 기존이 학습이 완료된 가중치 파라미터 값으로 바꿔치지 하는 것으로 이해할 수 있습니다.
